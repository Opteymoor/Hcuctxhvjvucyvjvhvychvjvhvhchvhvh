...
bot = Bot("AppName", auth="jyakdzjgxriwebrrgjupjqlelohvezci")
target ="g0BNopQ0cac9a1adfe570cad15f513c5"
bot.sendVoice(target, "/storage/emulated/0/roboot/power-rob-main/power-rob-main/Am.ogg",9000, caption= f"ربات ددپول فعال شده است🍆")
...
def remove_star(guid,user):
	...
	if remove_count == 2:
		...
		bot.sendMessage(target,   f"از طرف مدیر گرامی [3/3] امتیاز از شما {guil} کسر شد اکنون دیگه ادمین نخواهی بود فشار بخور", message_id=mesagid)
				...
				if not sleeped:
					if msg.get("text") == "/bot" and msg.get("author_object_guid") in admins :
						...
						try:
							...
							rules = open("poer.txt","r",encoding='utf-8').read()
							bot.sendMessage(target, str(rules), message_id=msg.get("message_id"))
							
					elif msg.get("text").startswith("اپدیت بازی") and msg.get("author_object_guid") in admins:
							try:
								rules = open("poer.txt","w",encoding='utf-8').write(str(msg.get("text").strip("اپدیت بازی")))
						...
						mop.close()
					elif msg.get("text") == "wrt" or msg.get("text") == "یاد" :
						try:
							data = msg.get("text").split("\n")
							f = open("data.txt","a",encoding="utf")
							f.write(str(data[1] + "|=|" + data[2] + "|/|" + "\n" ))
					...
					if msg.get("text") == "/بات" and msg.get("author_object_guid") in admins :
						...
						try:
							...
							bot.sendMessage(target, "لطفا دستور را به طور صحیح وارد کنید ❌", message_id=msg["message_id"])
												...
												bot.sendMessage(target, "فرشته 18 ساله دارای 85 از تهران", message_id=msg.get("message_id"))
							...
							try:
								bot.sendMessage(target, "خوب میخندی", message_id=msg.get("message_id"))
							except:
								print("err luagh")
					...
					elif msg.get("text") == "داد بزن":
						bot.sendVoice(target,"/storage/emulated/0/roboot/power-rob-main/power-rob-main/Ar.ogg",9000, message_id=msg.
get("message_id"))
												
					elif msg.get("text") == "به کیرم"or msg.get("text") == "بتخمم"or msg.get("text") == "به تخمم":
						bot.sendMessage(target, "از داشته هات مایع بزار کونی", message_id=msg.get("message_id"))	
						
					elif msg.get("text") == "میوسیوقیو":
						bot.sendVoice(target, "/storage/emulated/0/Download/wsup.mp3",207000, message_id=msg.get("message_id"))											
					...
					elif msg.get("text") == "حذف"or msg.get("text") == "پاک" and msg.get("author_object_guid") in admins :
						bot.deleteMessages(target, [msg.get("reply_to_message_id")])
						bot.sendMessage(target, "پیام پاک شد ✅", message_id=msg.get("message_id"))
					...
					elif msg.get("text") == ("ریم")or msg.get("text") == ("بن")or msg.get("text") == ("کون لقت") and msg.get("author_object_guid") in admins :
						try:
